...
open class TTGSnackbar: UIView {
    ...
    @objc public static var snackbarDefaultFrame: CGRect = CGRect(x: 0, y: 0, width: 320, height: 49) // (UIDevice.current.hasNotch ? 83 : 49))
    
    /// Snackbar min height
    @objc public static var snackbarMinHeight: CGFloat = 49 // (UIDevice.current.hasNotch ? 83 : 49)
    ...
    @objc open dynamic var shouldActivateLeftAndRightMargin: Bool = false
    ...
    @objc open dynamic var bottomMargin: CGFloat = (UIDevice.current.hasNotch ? (83 + .spacing8) : (49 + .spacing8)) {
        didSet {
    ...
    @objc open dynamic var leftMargin: CGFloat = .spacing8 {
        didSet {
    ...
    @objc open dynamic var rightMargin: CGFloat = .spacing8 {
        didSet {
    ...
    /// Icon width
    @objc open dynamic var iconImageViewWidth: CGFloat = 0 {
        didSet {
            iconImageViewWidth = min(iconImageViewWidth, 32)
...
public extension TTGSnackbar {
    ...
    @objc func show() {
        ...
        if let superView = containerView ?? currentWindow {
            ...
            leftMarginConstraint?.isActive = self.shouldActivateLeftAndRightMargin ? true : customContentView == nil
            rightMarginConstraint?.isActive = self.shouldActivateLeftAndRightMargin ? true : customContentView == nil
...
private extension TTGSnackbar {
    ...
    func configure() {
        ...
        actionButton.contentEdgeInsets = UIEdgeInsets(top: 2, left: 10, bottom: 2, right: 10)
        actionButton.titleLabel?.font = actionTextFont
        ...
        actionButton.applyBorder(with: UIColor.white, width: 1)
        actionButton.roundedCorner(radius: 2)
        ...
        let hConstraints = NSLayoutConstraint.constraints(
            withVisualFormat: "H:|-0-[iconImageView]-2-[messageLabel]-2-[seperateView(0.5)]-2-[actionButton(>=44@999)]-0-[secondActionButton(>=44@999)]-14-|",
        ...
        let vConstraintsForActionButton = NSLayoutConstraint.constraints(
            withVisualFormat: "V:|-10-[actionButton]-10-|",
            options: NSLayoutConstraint.FormatOptions(rawValue: 0),
            metrics: nil,
